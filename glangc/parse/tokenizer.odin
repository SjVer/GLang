...
// adapted from https://github.com/odin-lang/Odin/blob/master/core/odin/tokenizer/tokenizer.odin
...
init_tokenizer :: proc(t: ^Tokenizer, src: string, path: string) {
	...
	advance_rune(t)
	if t.ch == utf8.RUNE_BOM do advance_rune(t)
}
...
error :: proc(t: ^Tokenizer, offset: int, msg: string, args: ..any) {
	...
	report.error(Span{pos, pos}, msg, ..args)
}
...
scan :: proc(t: ^Tokenizer) -> Token {
	...
	// if .Insert_Semicolon in t.flags {
		...
		case .Ident, .Break, .Continue, .Return,
		     .Integer, .Float, .Char,
		     .Question, .Close_Paren, .Close_Bracket, .Close_Brace:
			/*fallthrough*/
